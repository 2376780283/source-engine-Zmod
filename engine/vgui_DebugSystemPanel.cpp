...
public:
	...
	int screenWc, screenHc;
    vgui::surface()->GetScreenSize(screenWc, screenHc);
		...
		if ( kv )
		{
			if ( kv->LoadFromFile(g_pFullFileSystem, "scripts/DebugOptions.txt") )
			{
				for (KeyValues *dat = kv->GetFirstSubKey(); dat != NULL; dat = dat->GetNextKey())
				{
					...
					{ 
					    if ( dat->GetInt() == 0 ) // 我不知道咋写了 反正Java中是这么写的
					    {
					        SetWide( screenWc );
					    }else{
					    	SetWide( dat->GetInt() );
	...
	: BaseClass( parent, panelName )
{
	...
	int screenWidth = videomode->GetModeStereoWidth();
    int menuWidth = 110;
    int margin = 50; // 右侧边距
    
    // 计算右上角位置：屏幕宽度 - 菜单宽度 - 边距
    m_pDebugMenu->SetPos(screenWidth - menuWidth - margin, margin); 
